...
State_Trotting::State_Trotting(CtrlComponents *ctrlComp)
    ...
    _gait = new GaitGenerator(ctrlComp); // GaitGenerator类的指针，生成期望位置和速度：_posFeetGlobalGoal 和 _velFeetGlobalGoal。
...
void State_Trotting::enter(){ // 初始化目标值
...
void State_Trotting::run(){
    ...
    _B2G_RotMat = _lowState->getRotMat(); // R_sb 机身姿态，世界 -> 机身。Eigen::Matrix<double, 3, 3>
    _G2B_RotMat = _B2G_RotMat.transpose(); // R_bs , 机身 -> 世界。 Eigen::Matrix<double, 3, 3>
...
void State_Trotting::calcTau(){ // 计算期望足端力。                                 
    ...
    _ddPcd(0) = saturation(_ddPcd(0), Vec2(-3, 3)); // 机身在世界坐标s下的目标线加速度。
    _ddPcd(1) = saturation(_ddPcd(1), Vec2(-3, 3)); // 这里用saturation限定最大最小值。
    _ddPcd(2) = saturation(_ddPcd(2), Vec2(-5, 5));

    _dWbd(0) = saturation(_dWbd(0), Vec2(-40, 40)); // 机身在世界坐标s下的目标角加速度。
    _dWbd(1) = saturation(_dWbd(1), Vec2(-40, 40));
    _dWbd(2) = saturation(_dWbd(2), Vec2(-10, 10));

    _forceFeetGlobal = - _balCtrl->calF(_ddPcd, _dWbd, _B2G_RotMat, _posFeet2BGlobal, *_contact); // 在世界坐标s下的期望足端力。

    for(int i(0); i<4; ++i){
        if((*_contact)(i) == 0){
            _forceFeetGlobal.col(i) = _KpSwing*(_posFeetGlobalGoal.col(i) - _posFeetGlobal.col(i)) + _KdSwing*(_velFeetGlobalGoal.col(i)-_velFeetGlobal.col(i)); // 对摆动腿的kd控制。
        }
    }

      // 在机身坐标b下的期望足端力。
    _q = vec34ToVec12(_lowState->getQ()); // 各个关节的现时角度。
    _tau = _robModel->getTau(_q, _forceFeetBody); // 各个关节的前馈力矩。
}

void State_Trotting::calcQQd(){ //根据目标足端位置和速度 -> 各个关节的角度和角速度。

    Vec34 _posFeet2B;
    _posFeet2B = _robModel->getFeet2BPositions(*_lowState,FrameType::BODY); //现时的足端位置, 在机身坐标b下。
    
    for(int i(0); i<4; ++i){
        _posFeet2BGoal.col(i) = _G2B_RotMat * (_posFeetGlobalGoal.col(i) - _posBody); // 期望的足端位置，在机身坐标b下。
        _velFeet2BGoal.col(i) = _G2B_RotMat * (_velFeetGlobalGoal.col(i) - _velBody); // 期望的足端速度，在机身坐标b下。
    ...
    _qGoal = vec12ToVec34(_robModel->getQ(_posFeet2BGoal, FrameType::BODY)); // 各个关节的目标角度，在机身坐标b下。
    _qdGoal = vec12ToVec34(_robModel->getQd(_posFeet2B, _velFeet2BGoal, FrameType::BODY)); // 各个关节的目标角速度，在机身坐标b下。
}
